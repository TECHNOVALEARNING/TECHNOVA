...
const CheckoutDialog = ({
  ...
  const cardType = useMemo(() => {
    ...
    if (
      ...
      /^222[1-9]|^22[3-9]|^2[3-6]|^27[0-1]|^2720/.test(cleanNumber)
    )
      return "mastercard";
    return "unknown";
  }, [cardNumber]);
  ...
  const [country, setCountry] = useState<PawaPayCountry>(
    () =>
      pawapayCountries.find((c) => c.code === (map2LetterTo3Letter[countryCode] || "BEN")) ||
      pawapayCountries[0],
  );
  ...
  const discountedPrice = (() => {
    ...
    if (appliedPromo.discount_percent)
      return Math.max(0, Math.round(effectivePrice * (1 - appliedPromo.discount_percent / 100)));
    if (appliedPromo.discount_amount)
      return Math.max(0, effectivePrice - appliedPromo.discount_amount);
  ...
  const handleApplyPromo = async () => {
    ...
    try {
      ...
      if (productIds && productIds.length > 0 && !productIds.includes(product.id)) {
        toast.error("Non applicable");
        return;
      }
      setAppliedPromo({
        code: data.code,
  ...
  const shippingPayload = needsShipping
    ? {
        ...
        country: shipCountry.trim() || country.name,
      }
    : null;
  ...
  const handleNext = () => {
    ...
    if (!phone.trim() || phone.length < 6) {
      toast.error("Téléphone valide requis");
      return;
    }
    if (needsShipping) {
      if (!shipAddress.trim()) {
              ...
              <div className="flex items-center gap-3 mb-6">
                {[
                  ...
                  <div key={s.n} className="flex items-center gap-2 flex-1 min-w-0">
                    ...
                    <div
                      className={`text-[11px] font-semibold truncate ${step >= s.n ? "text-gray-900" : "text-gray-400"}`}
                    >
                      {s.label}
                    </div>
                  </div>
              ...
              {step === 1 && (
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="Doe"
                        className="h-11 bg-muted/30 border-border/60 focus:bg-card text-foreground text-sm"
                      />
                    </div>
                  </div>
                        ...
                        <PopoverContent className="w-72 p-0" align="start">
                          <div className="p-2 border-b border-border">
                            <Input
                              ...
                              className="h-8 text-sm bg-muted/30 border-border/60 text-foreground"
                            />
                          </div>
                  ...
                  {needsShipping && (
                    <div className="space-y-3 rounded-xl border border-border/60 bg-muted/20 p-3">
                      ...
                      <Input
                        ...
                        className="h-11 bg-card border-border/60 text-foreground"
                      />
                      ...
                      <Input
                        ...
                        placeholder={`Pays (par défaut: ${country.name})`}
                        className="h-11 bg-card border-border/60 text-foreground"
                      />
                    </div>
                  )}
                      ...
                      {!showPromo && !appliedPromo ? (
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <div className="text-xs text-emerald-600 dark:text-emerald-500">
                                -{savings.toLocaleString()} {currency} économisés
                              </div>
                            </div>
                          </div>
                          <button
                            type="button"
                            onClick={removePromo}
                            className="text-emerald-600 dark:text-emerald-500 hover:text-emerald-800"
                          >
                            <X className="h-4 w-4" />
                          </button>
                        </motion.div>
                        ...
                        <div className="flex gap-2">
                          <Input
                            ...
                            className="flex-1 h-10 uppercase font-mono bg-muted/30 border-border/60 text-foreground"
                          />
                          <Button
                            type="button"
                            size="sm"
                  ...
                  <div className="flex items-center justify-center gap-4 text-[11px] text-muted-foreground pt-1">
                    ...
                    <span className="flex items-center gap-1">
                      <ShieldCheck className="h-3 w-3" /> Données chiffrées
                    </span>
                  </div>
                </motion.div>
              )}
        ...
        >
          ...
          <div className="relative z-10 flex-1 flex flex-col">
            ...
            <div className="space-y-3 mb-auto">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-semibold">
                  {effectivePrice.toLocaleString()} {currency}
                </span>
              </div>
            ...
            <div className="flex items-end justify-between mb-5">
              ...
              {!isFree && (
                <div className="text-sm font-semibold text-white/80 mb-1">{currency}</div>
              )}
            </div>
            ...
            <div className="mt-5 pt-4 border-t border-white/10">
              <div className="text-[9px] uppercase tracking-widest text-white/50 mb-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                {providerLogosForStrip.map(([k, src]) => (
                  <div
                    key={k}
                    className="h-6 w-9 rounded bg-white/95 p-0.5 flex items-center justify-center"
                  >
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent
        className={`${step === 3 ? "sm:max-w-[460px]" : "sm:max-w-[940px]"} p-0 overflow-hidden border-0 bg-transparent shadow-2xl max-h-[95vh] sm:max-h-[92vh] transition-all duration-300`}
      >
        {innerContent}
      </DialogContent>
    </Dialog>
        ...
        >
          <div
            ...
            style={{
              ...
              filter: `drop-shadow(0 12px 30px ${accent}80)`,
            }}
          />
      ...
      <ol className="space-y-2 text-left w-full max-w-sm text-xs text-muted-foreground mb-4">
        {[
          ...
          <li key={i} className="flex items-start gap-2">
            <div
              className="h-4 w-4 rounded-full text-white flex items-center justify-center text-[9px] font-bold shrink-0 mt-0.5"
              style={{ backgroundColor: accent }}
            >
              {i + 1}
            </div>
