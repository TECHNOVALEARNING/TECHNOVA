...
const guessCategory = (title: string): string => {
  ...
  if (
    ...
    t.includes("react") ||
    t.includes("node") ||
    t.includes("technicien")
  ) {
    return "Tech";
  }
  if (
    t.includes("design") ||
    t.includes("graphiste") ||
    t.includes("ux") ||
    t.includes("ui") ||
    t.includes("infographe")
  ) {
    return "Design";
  }
  if (
...
const Jobs = () => {
  ...
  const filteredJobs = displayedJobsList.filter((job) => {
    ...
    const locMatch = job.location[lang as "fr" | "en"]
      .toLowerCase()
      .includes(location.toLowerCase());

    const catMatch = category === "All" || job.category === category;
    const eduMatch =
      education === "All" || job.education.toLowerCase().includes(education.toLowerCase());

    let expMatch = true;
    if (experience !== "All") {
      if (experience === "Beginner") {
        expMatch =
          job.experience.en.toLowerCase().includes("begin") ||
          job.experience.fr.toLowerCase().includes("début");
      } else {
        expMatch =
          job.experience.en.toLowerCase().includes(experience.toLowerCase()) ||
          job.experience.fr.toLowerCase().includes(experience.toLowerCase());
      }
    }
  ...
  return (
    <div className="min-h-screen bg-background font-sans transition-colors duration-300" style={{ fontFamily: "'Manrope', -apple-system, sans-serif" }}>
      <SEOHead
        title={
          lang === "fr"
            ? "Offres d'Emplois & Stages - TechNova Learning"
            : "Jobs & Internships - TechNova Learning"
        }
        description={
          lang === "fr"
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
              ...
              <div className="md:col-span-5 relative">
                ...
                <input
                  type="text"
                  placeholder={
                    lang === "fr" ? "Métier, Mots-clés, Entreprise..." : "Job, Keywords, Company..."
                  }
                  value={search}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      >
                        ...
                        <option value="Tech">
                          {lang === "fr" ? "Tech & Développement" : "Tech & Dev"}
                        </option>
                    ...
                    <div className="space-y-1.5">
                      ...
                      >
                        ...
                        <option value="Beginner">
                          {lang === "fr" ? "Débutant (0-1 an)" : "Beginner (0-1 yr)"}
                        </option>
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between border-b border-border/60 pb-4">
              ...
              <span className="text-sm font-medium text-muted-foreground bg-secondary/60 px-3 py-1 rounded-full border border-border/40">
                {isLoadingLive ? "..." : filteredJobs.length}{" "}
                {lang === "fr" ? "offres disponibles" : "jobs found"}
              </span>
            </div>
            ...
            {isLoadingLive ? (
              <div className="space-y-4">
                {[...Array(3)].map((_, i) => (
                  <div
                    key={i}
                    className="p-6 rounded-2xl bg-card border border-border/80 animate-pulse space-y-3"
                  >
              ...
              <div className="text-center py-16 bg-card border border-border/60 rounded-2xl">
                ...
                <h3 className="text-lg font-bold text-foreground mb-1">
                  {lang === "fr"
                    ...
                    : "No job listings matched your filters"}
                </h3>
