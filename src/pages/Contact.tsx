...
const translations = {
  fr: {
    ...
    seoDesc: "Contactez l'équipe TECHNOVA. Support disponible 24/7 pour répondre à toutes vos questions sur la vente de produits digitaux.",
  ...
  en: {
    ...
    seoDesc: "Contact the TECHNOVA team. Support is available 24/7 to answer all your questions about selling digital products.",
        ...
        <div className="container mx-auto px-6 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <h1 className="text-4xl md:text-6xl font-extrabold text-foreground mb-6">
              <span className="text-gradient">{t.heading.split("-")[0]}</span>{t.heading.includes("-") ? `-${t.heading.split("-")[1]}` : ""}
            </h1>
          ...
          <div className="grid gap-16 md:grid-cols-2 max-w-4xl mx-auto">
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <Textarea placeholder={t.messagePlaceholder} value={message} onChange={(e) => setMessage(e.target.value)} rows={5} required />
                <Button type="submit" className="w-full">{t.sendBtn}</Button>
              </form>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="space-y-8">
              ...
              {[
                { icon: Mail, title: "Email", desc: "contact@technova.com", href: "mailto:contact@technova.com" },
