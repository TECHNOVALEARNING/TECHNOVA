...
const AdminModeration = () => {
  ...
  const toggleProduct = async (id: string, isPublished: boolean) => {
    await supabase.functions.invoke("admin-platform", {
      body: { action: "toggle_product", productId: id, isPublished },
    });
  ...
  const toggleStore = async (id: string, isArchived: boolean) => {
    await supabase.functions.invoke("admin-platform", {
      body: { action: "archive_store", storeId: id, isArchived },
    });
  ...
  const getName = (
    p: { display_name: string | null; first_name: string | null; last_name: string | null } | null,
  ) => {
    if (!p) return "Inconnu";
  ...
  const filteredStores = stores.filter((s) => {
    ...
    return (
      ...
      getName(s.owner).toLowerCase().includes(q)
    );
  });
      ...
      <div className="space-y-5">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Gérer les produits et boutiques de la plateforme
          </p>
        </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {getName(p.creator)} · {typeLabel[p.type] || p.type} ·{" "}
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              Cette action est irréversible. Le produit "{p.title}" sera
                ...
                {filteredStores.map((s, i) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {getName(s.owner)} · /{s.slug} ·{" "}
                        {format(new Date(s.created_at), "dd MMM yyyy", { locale: fr })}
                      </p>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <Badge
                        variant={s.is_archived ? "destructive" : "default"}
                        className="text-[10px]"
                      >
