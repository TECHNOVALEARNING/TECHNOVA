...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!verification) {
      return new Response(JSON.stringify({ error: "No verification found for user" }), {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!targetSessionId) {
      return new Response(JSON.stringify({ error: "No session ID found" }), {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const diditRes = await fetch(
      `https://verification.didit.me/v3/session/${targetSessionId}/decision/`,
      {
        method: "GET",
        headers: {
          "x-api-key": apiKey,
    ...
    if (!diditRes.ok) {
      ...
      return new Response(
        JSON.stringify({ error: "Failed to fetch status from Didit: " + errText }),
    ...
    if (newStatus === "approved" && documentNumber) {
      ...
      if (dup) {
        newStatus = "rejected";
        rejectionReason =
          "Ce document a déjà été utilisé pour vérifier un autre compte TECHNOVA. Une personne ne peut vérifier qu'un seul compte.";
        console.warn("Duplicate KYC document detected", {
          documentNumber,
          attemptedUser: user.id,
          existingUser: dup.user_id,
        });
      }
    }
    ...
    if (newStatus !== verification.status || bodyPayload.sessionId) {
      ...
      await adminClient.from("identity_verifications").update(updateData).eq("user_id", user.id);
