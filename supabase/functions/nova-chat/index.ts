...
const tools = [
  {
    ...
    function: {
      ...
      description:
        "Récupère les statistiques de ventes du vendeur (total, nombre, top produits, top pays, période).",
      parameters: {
        type: "object",
        properties: {
          period_days: {
            type: "number",
            description: "Période en jours (7, 30, 90, 365). Défaut 30.",
          },
  ...
  },
  {
    ...
    function: {
      name: "list_products",
      description:
        "Liste les produits du vendeur (id, titre, prix, type, ventes). Utiliser quand l'utilisateur veut créer un code promo ou parler d'un produit.",
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Crée un code promo. Demande confirmation à l'utilisateur AVANT d'appeler cette fonction (code, % de réduction, produits choisis, expiration, max utilisations).",
      parameters: {
        type: "object",
        properties: {
          ...
          discount_percent: {
            type: "number",
            description: "% de réduction (1-100). Utiliser ceci OU discount_amount.",
          },
          discount_amount: {
            type: "number",
            description: "Montant fixe en FCFA. Utiliser ceci OU discount_percent.",
          },
          product_ids: {
            type: "array",
            items: { type: "string" },
            description: "Liste d'IDs produits concernés (vide = tous).",
          },
  ...
  },
  {
    ...
    function: {
      name: "audit_store",
      description:
        "Audit complet de la boutique : produits, prix, descriptions, ventes. Renvoie données brutes pour analyse.",
      parameters: { type: "object", properties: {} },
    },
  },
];
...
async function executeTool(name: string, args: any, userId: string) {
  ...
  if (name === "get_sales_stats") {
    ...
    const q = sb
      .from("orders")
      .select("amount,status,created_at,product_id,shipping_address,payment_method")
    ...
    const topProducts = topProductIds.map((id) => ({
      id,
      title: prods?.find((p) => p.id === id)?.title,
      revenue: byProduct[id],
    }));
    return {
      period_days: days,
      total_revenue_fcfa: total,
  ...
  if (name === "list_products") {
    const { data } = await sb
      .from("products")
      .select("id,title,price,type,sales_count,is_published")
  ...
  if (name === "create_promo_code") {
    ...
    if (args.expires_in_days)
      insert.expires_at = new Date(Date.now() + args.expires_in_days * 86400000).toISOString();
  ...
  if (name === "audit_store") {
    const { data: products } = await sb
      .from("products")
      .select("id,title,price,description,thumbnail_url,is_published,sales_count,type")
      .eq("creator_id", userId);
    ...
    return {
      ...
      products_without_thumbnail:
        products?.filter((p) => !p.thumbnail_url).map((p) => p.title) || [],
      products_short_description:
        products?.filter((p) => !p.description || p.description.length < 100).map((p) => p.title) ||
        [],
...
- Répondre aux questions générales sur Technova
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (thread_id && lastUser) {
      await sb
        .from("nova_messages")
        .insert({ thread_id, user_id: user.id, role: "user", content: { text: lastUser.content } });
      ...
      if (t?.title === "Nouvelle conversation") {
        await sb
          .from("nova_threads")
          .update({ title: String(lastUser.content).slice(0, 60) })
          .eq("id", thread_id);
      } else {
        await sb
          .from("nova_threads")
    ...
    if (thread_id && finalText) {
      await sb.from("nova_messages").insert({
        ...
        user_id: user.id,
        role: "assistant",
        content: { text: finalText, tools: toolCallsLog },
      });
    }

    return new Response(JSON.stringify({ message: finalText, tools: toolCallsLog }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e) {
