...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!corridor) {
      return j(
        { error: `Opérateur ${wallet.provider_code} non opérationnel pour les retraits` },
        400,
      );
    }
    ...
    if (amount > available)
      return j(
        { error: `Solde insuffisant. Disponible: ${Math.floor(available)} ${corridor.currency}` },
        400,
      );
    ...
    if (!resp.ok) {
      const reason =
        data?.message || data?.error || JSON.stringify(data?.errors || {}) || "Erreur Moneroo";
    ...
    await admin
      .from("withdrawals")
      .update({ moneroo_payout_id: monerooPayoutId })
      .eq("id", withdrawal.id);
