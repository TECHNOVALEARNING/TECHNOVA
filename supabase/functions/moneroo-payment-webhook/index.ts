...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (webhookSecret) {
      ...
      if (!ok)
        console.warn("[moneroo-payment-webhook] signature invalide", {
    ...
    if (!order && monerooId) {
      const { data: o } = await supabase
        ...
        .eq("moneroo_transaction_id", monerooId)
        .maybeSingle();
      order = o;
    }
    if (!order) {
    ...
    if (isSuccess && order.status !== "completed") {
      await supabase
        ...
        .update({
          status: "completed",
          moneroo_transaction_id: monerooId || order.moneroo_transaction_id,
        })
        .eq("id", order.id);
