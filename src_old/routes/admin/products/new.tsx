...
function NewProduct() {
  ...
  const canNext = () => {
    switch (step) {
      ...
      case 3: return true;
      case 4:
        if (selectedType === 'file') return downloadFiles.length > 0;
              ...
              {selectedType === 'file' && (
                <div className="space-y-6">
                  <div
                    ...
                    className="border-2 border-dashed border-slate-300 rounded-2xl p-12 flex flex-col items-center justify-center cursor-pointer hover:border-amber-500 hover:bg-amber-50/50 transition-all"
                  >
                    ...
                    {downloadFiles.length > 0 && (
                      <div className="mt-6 w-full text-left space-y-2">
                        {downloadFiles.map((file, idx) => (
                          <div key={idx} className="flex justify-between items-center text-[13px] font-bold text-[#1E293B] bg-white p-3 rounded-xl border border-slate-200">
                            <span className="truncate pr-4">📄 {file.name}</span>
                            <button 
                              type="button" 
                              onClick={(e) => { e.stopPropagation(); setDownloadFiles(downloadFiles.filter((_, i) => i !== idx)); }}
                    ...
                    <input
                      ...
                      onChange={e => {
                        if (e.target.files) {
                          setDownloadFiles(prev => [...prev, ...Array.from(e.target.files!)]);
                        }
                      }}
                    />
                  </div>
