                    ...
                    <div className="space-y-3">
                      {(() => {
                        ...
                        if (fileUrls.length === 0) {
                          return <div className="text-slate-500 italic p-4 bg-white rounded-2xl border border-slate-200">Aucun fichier disponible</div>;
                        }

                        return fileUrls.map((url: string, idx: number) => (
                          <div key={idx} className="bg-white rounded-2xl border border-slate-200 p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm">
                            <div className="flex items-center gap-4">
                              <div className="w-12 h-12 bg-slate-100 rounded-xl flex items-center justify-center text-slate-500 shrink-0">
                                <FileText className="w-6 h-6" />
                              </div>
                              <div className="min-w-0">
                                <h3 className="font-semibold text-slate-900 truncate">Accès {product.title}</h3>
                                <p className="text-sm text-slate-500 truncate">{url.split('/').pop() || 'Fichier'}</p>
                              </div>
                            </div>
                            <div className="flex items-center gap-2 shrink-0">
                              <a href={url} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-700 font-medium rounded-xl hover:bg-slate-50 transition-colors">
                                <Download className="w-4 h-4" />
                                {fileUrls.length > 1 ? `Télécharger Fichier ${idx + 1}` : 'Télécharger'}
                              </a>
                            </div>
                          </div>
                        ));
